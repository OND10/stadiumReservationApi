...
namespace Reservationpitch.Application.Services.User.Implementation
{
    public class UserService : IUserService
    {
        ...
        public async Task<Result<LoginResponseDto>> Login(LoginRequestDto request)
        {
            ...
            if (user != null)
            {
                var validPassword = await _repository.CheckUserPasswordAsync(user, request.Password);

                if (validPassword)
                {
                    var roles = await _repository.GetUserRolesAsync(user);

                    // ✅ NEW: Add claims based on the roles
                    //await _repository.AddClaimsToUser(user);

                    // ✅ Then generate token with claims
                    var tokens = await _tokenRepository.CreateJWTTokenAsync(user, roles.ToList());
        ...
        public async Task<Result<RefreshTokenResponseDto>> RefreshTokenAsync(RefreshTokenRequestDto request)
        {
            ...
            var roles = await _repository.GetUserRolesAsync(user);
            var newAccessToken = await _tokenRepository.CreateJWTTokenAsync(user, roles);
        ...
        public async Task<Result<bool>> SendToAllUsersAsync(string message)
        {
            var result = await _repository.SendToAllUsers(message);

            return await Result<bool>.SuccessAsync(true, ResponseStatus.SendedSuccess, true);
        }

        public async Task<Result<IEnumerable<string>>> GetUserMessagesAsync(string userId)
        {
            var result = await _repository.GetUserMessages(userId);

            // Mark notifications as read after fetching them
            await _repository.MarkMessagesAsRead(userId);

            return await Result<IEnumerable<string>>.SuccessAsync(result, ResponseStatus.GetSuccess, true);
        }

        public async Task<Result<int>> GetUserMessagesCount(string userId)
        {
           var result = await _repository.GetMessagesCount(userId);

            return await Result<int>.SuccessAsync(result, ResponseStatus.GetSuccess, true);
        }
    }
